    ...
    <div className="px-75 mt-15">
      <div className='flex items-end justify-between'>
        <div>
          ...
          <p className='font-hankengrotesk italic text-orange text-[50px] leading-[60px] mt-2'>what matters.</p>
        </div>
        <p className='w-125 text-lg text-gray-400 leading-8 mb-4'>These aren't slogans. They're the operating rules of a company built by someone who lived every problem the system creates.</p>
      </div>
      ...
      <div className='mt-12 grid grid-cols-2 border-t border-white/10'>
        {items.map((item, i) => (
          <div
            key={item.number}
            className={[
              'flex gap-6 px-8 py-10',
              'border-b border-white/10',
              i % 2 === 0 ? 'border-r border-white/10' : '',
            ].join(' ')}
          >
            <span className='font-bigshoulders font-bold text-6xl text-orange leading-none shrink-0 w-16'>
              {item.number}
            </span>
            <div>
              <h3 className='font-bigshoulders font-bold text-xl uppercase tracking-wide mb-3'>
                {item.title}
              </h3>
